const parseInput = () => {
  ...
  return input.map(line => {
    ...
    return { sue: m[1], [m[2]]: m[3], [m[4]]: m[5], [m[6]]: m[7] };
  });
}
...
const qualified = auntSue =>
  Object.keys(auntSue).filter(k => k !== "sue").every(k => auntSue[k] === auntSueTarget[k]);

const part2 = (key, value) => {
  switch (key) {
    case "cats":
    case "trees":
      return value > auntSueTarget[key];
    case "pomeranians":
    case "goldfish":
      return value < auntSueTarget[key];
    default:
...
const qualifiedPart2 = auntSue =>
  Object.keys(auntSue).filter(k => k !== "sue").every(k => part2(k, auntSue[k]));

console.log(input.filter(qualified));
console.log(input.filter(qualifiedPart2));
