...
namespace foo { class bar {
        ...
        while (i < s.Length) {
            if (s[i] != '(') {
                ...
                while (Char.IsDigit(s[i])) {
                    numChars = 10 * numChars + Int32.Parse(s[i].ToString());
                    i++;
                }
                ...
                var repeatCount = 0;
                while (Char.IsDigit(s[i])) {
                    repeatCount = 10 * repeatCount + Int32.Parse(s[i].ToString());
                    i++;
                }

                i++; // Skip ')'

                // Emit the repeated characters
    ...
    // Part 2
    static long DecodedLength(string s) {
        long length = 0;
        int i = 0;
        ...
        while (i < s.Length) {
            if (s[i] != '(') {
                ...
                length += repeatCount * DecodedLength(s.Substring(i, numChars));

                i += numChars; // Skip the input we just recursed on
            }
        }
    ...
    static void Main() {
        // Part 1
        Console.WriteLine(Decode(input).Count());

        // Part 2
        Console.WriteLine(DecodedLength(input));
    }
}}
